...
export default function SignupPage() {
    ...
    type FieldName = 'fullName' | 'email' | 'password' | 'confirmPassword' | 'agreeTerms';

    const validateField = (name: FieldName, value: string | boolean): string => {
        switch (name) {
            case "fullName":
            ...
            case "password":
                if (typeof value !== "string" || !value.trim()) return "Password is required";
            ...
            case "confirmPassword":
                if (typeof value !== "string" || !value.trim()) return "Please confirm your password";
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (touched[fieldName]) {
            setErrors(prev => ({ ...prev, [fieldName]: validateField(fieldName, fieldValue) }));
        }
        ...
        <div className="h-screen flex overflow-hidden">
            ...
            <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-orange-500 to-orange-600 items-center justify-center p-12 relative overflow-hidden">
                ...
                <div className="text-center z-10">
                    <div className="mb-8">
                        <div className="w-50 h-50 bg-white rounded-full mx-auto flex items-center justify-center shadow-2xl relative">
                            <div className="absolute inset-0 rounded-full border-4 border-orange-500 animate-ping"></div>
                            <Image src="/svgs/brainburnerslogo.svg" alt="The Brain Burners" width={140} height={40} priority className="relative" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center bg-white overflow-y-auto">
                <div className="w-full max-w-md px-6 py-2 md:py-8">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-black text-lg font-medium">Sign Up</span>
                    </div>

                    <h2 className="text-2xl sm:text-4xl text-black font-black mb-1">Create Your Account</h2>
                    <p className="text-gray-600 mb-6 text-sm">Fill in your details to get started</p>

                    {submitError && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-4 text-sm">{submitError}</div>}
                    {submitSuccess && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl mb-4 text-sm">Account created successfully! Please check your email to verify your account.</div>}

                    <form onSubmit={handleSubmit} className="space-y-4">
                        {/* Full Name */}
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">Full Name</label>
                            <input type="text" name="fullName" placeholder="John Doe" value={formData.fullName} onChange={handleChange} onBlur={handleBlur} disabled={isSubmitting}
                        ...
                        <div>
                            ...
                            <input type="email" name="email" placeholder="your@email.com" value={formData.email} onChange={handleChange} onBlur={handleBlur} disabled={isSubmitting}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input type={showPassword ? "text" : "password"} name="password" placeholder="Create a strong password" value={formData.password} onChange={handleChange} onBlur={handleBlur} disabled={isSubmitting}
                                ...
                                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                                    {showPassword ? (
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword" placeholder="Re-enter your password" value={formData.confirmPassword} onChange={handleChange} onBlur={handleBlur} disabled={isSubmitting}
                                ...
                                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                                    {showConfirmPassword ? (
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                        </svg>
                        ...
                        <div>
                            <label className="flex items-start">
                                <input type="checkbox" name="agreeTerms" checked={formData.agreeTerms} onChange={handleChange} onBlur={handleBlur} className="w-4 h-4 mt-0.5 text-orange-500 border-gray-300 rounded focus:ring-orange-500" />
                            ...
                            {errors.agreeTerms && touched.agreeTerms && <p className="text-red-500 text-xs mt-1">{errors.agreeTerms}</p>}
                        </div>

                        <button type="submit" disabled={isSubmitting} className="bg-orange-500 hover:bg-orange-600 w-full rounded-xl p-2.5 text-white font-medium text-sm disabled:opacity-50">
                            {isSubmitting ? "Creating Account..." : "Create Account"}
                        </button>

                        <p className="text-xs mt-3 text-center text-gray-600">
                            Already have an account? <Link href="/login" className="text-orange-500 font-medium hover:underline">Log in</Link>
                        </p>
                    </form>
                </div>
            </div>
